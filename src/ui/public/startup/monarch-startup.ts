export interface MonarchStartupOptions {
  title?: string;
  subtitle?: string;
...
export class MonarchStartup {
  ...
  private readonly options: Required<MonarchStartupOptions>;

  constructor(options: MonarchStartupOptions = {}) {
    this.options = {
  ...
  public mount(): void {
    ...
    this.progressValueElement = root.querySelector(
      "[data-monarch-progress-value]",
    );
  ...
  public async complete(finalStatus = "Monarch готов"): Promise<void> {
    if (this.isCompleted) {
      return;
    }
    ...
    const remainingTime = Math.max(
      0,
      this.options.minimumVisibleTime - elapsed,
    );
  ...
  public fail(message = "Ошибка запуска"): void {
    this.stopAutoProgress();
  ...
  private startAutoProgress(): void {
    ...
    this.autoProgressTimer = window.setInterval(() => {
      ...
      const increment = Math.max(
        0.0015,
        remaining * (0.025 + Math.random() * 0.035),
      );
  ...
  private createAnimatedTitle(title: string): string {
    return [...title]
      .map((character, index) => {
        const safeCharacter =

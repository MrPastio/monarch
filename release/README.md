...
`release/stable-release-spec.json` is deliberately set to `available: false`, so the workflow refuses to publish. After the installer/runtime/launcher contracts are real:
...
4. Replace the draft notes in `release/notes/v0.1.5.md`.

...
`release/stable-release-spec.json` contains the accepted immutable runtime/environment IDs and an armed `available: true` stable release. Publish only from the exact reviewed commit by running `Stable release` with version `0.2.3.7`.

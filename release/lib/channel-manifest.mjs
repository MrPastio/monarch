...
function assertSemver(value, path) {
  ...
  if (!SEMVER_PATTERN.test(value)) fail(`${path} must be a SemVer value without a leading v`);
}
...
export function validateChannelManifest(manifest) {
  ...
  assertSemver(manifest.minimumLauncherVersion, 'minimumLauncherVersion');
  ...
  for (const [index, version] of manifest.revokedVersions.entries()) {
    assertSemver(version, `revokedVersions[${index}]`);

...
export function compareSemver(left, right) {
  ...
  if (!a || !b) throw new UpdateServiceError('invalid-semver', 'Version is not a valid stable Monarch version.');
...
function validateManifest(value, { now, expectedChannel, maxInstallerBytes }) {
  ...
  if (!parseSemver(version)) throw new UpdateServiceError('invalid-semver', 'Manifest version is not a stable Monarch version.');
...
function parseSemver(value) {
  const match = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?$/.exec(String(value || ''));
...
function readSafeInstallerName(value) {
  ...
  if (
    ...
    || !/^Monarch-Setup-\d+\.\d+\.\d+(?:\.\d+)?\.exe$/i.test(fileName)
  ) {

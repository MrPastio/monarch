...
- Voice input is intentionally bounded for quick local STT: minimum 500 ms, maximum 12 s, maximum 8 MB client-side audio blob. This prevents accidental long recordings from hitting the backend STT timeout.
...
- Before insertion, the renderer restores spoken punctuation and sentence casing for RU/UK/BG/EN (`точка`, `знак вопроса`, `comma`, `question mark`, and equivalents). Question endings are inferred conservatively; URLs, file names, and decimal numbers are left intact.

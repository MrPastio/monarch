...
- The normal Desktop engine is the local `Qwen3-TTS-12Hz-0.6B-Base` model conditioned on one of three bundled references: `oscar-reference.wav` (baritone), `oscar-clear-reference.wav` (clear male), or `aurora-reference.wav` (warm female). `assets/voice/reference-provenance.json` defines synthetic Russian replacements designed from text plus natural-language instructions in Qwen VoiceDesign, with no human reference recording or voice-cloning input. That provenance claim applies to promoted bytes only after the contract is `verified` and the full release check passes; `pending-regeneration` deliberately makes no claim about existing bundled bytes. The pinned model revision, exact model-file hashes, canonical Russian text, descriptions, seeds, settings, and output-evidence status live in that contract; safe offline regeneration is documented in `assets/voice/PROVENANCE.md`. `tools/local-neural-tts.py` keeps the model, per-reference prompt cache, and captured CUDA graphs alive, splits the full answer at semantic boundaries, streams audio into a queue-backed PortAudio output, and accepts stop/new-request commands over newline-delimited JSON.

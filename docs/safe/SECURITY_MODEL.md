...
- The Safe renderer has Node integration disabled, context isolation and Chromium sandbox enabled, no network permissions, no popups or navigation, no external program launch, and an ephemeral Electron session partition. Clipboard access is blocked by default; the signed user policy can allow copy-only or read/write access without changing the network/process boundary.
- The signed Safe policy controls inactivity reauthentication, clipboard mode, clipboard cleanup, focus-loss locking, and minimize behavior. Saving any policy requires the current Safe PIN. A policy assessed as low security also requires a separate explicit acknowledgement before it is committed.
- Locking clears rendered metadata, credential/editor values, object URLs, and in-process key buffers that Monarch directly controls. Auto-lock is serialized with active vault operations, so it cannot invalidate the manifest halfway through a commit. By default, successful manual/automatic lock, window hiding/minimizing, OS lock, and suspend destroy the Safe window and utility process; a user may choose lock-on-minimize or keep-unlocked-on-minimize after the policy confirmation flow. OS lock and suspend remain mandatory system boundaries.
...
- File deletion and three-attempt vault destruction perform active-key erasure first and unlink unreferenced ciphertext blobs second. Safe intentionally does not overwrite or truncate ciphertext through its path: a hostile same-volume hardlink must not let vault cleanup modify an unrelated file. Recovered historical manifests or backups remain part of the physical-storage limitation below.
...
- No desktop application can promise physically irreversible overwrite on every SSD, filesystem snapshot, backup, pagefile, crash dump, or previously copied source. Monarch therefore treats key destruction as the erasure guarantee and does not claim physical overwrite.
...
- The focused Safe regression suite covers plaintext-at-rest absence, hidden authenticated chat records, tamper failure, lock denial, bridge allowlisting/trust checks, generation rotation, rollback semantics, policy confirmation/PIN enforcement, hardlink-safe deletion, folder/section boundaries, and auto-lock/write serialization.
- `desktop/safe/qa.mjs` exercises the real Electron renderer/runtime boundary with Windows `safeStorage`: 4/6/12-cell UI switching, setup and recovery display, creation from the all-files view, direct deletion, section customization, file/archive flows, low-policy warning and strong-policy restoration, encrypted-chat upsert/list/read/delete through the utility-process service, image/audio/video/PDF/HEX policies, network/popup/clipboard denial, signed write/delete gates, one auto-lock event, and plaintext/credential cleanup after lock.
